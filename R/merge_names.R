...
#'
setMethod(
  "merge_names", signature(object = "PostgreSQLConnection"),
  function(object, new_id, old_id,
    ...
    if (any(n2c)) {
      stop(paste0(
        "Replacement will violate double entries ",
    ...
    for (i in 1:length(schemas)) {
      Query <- paste0(
        "UPDATE \"", schemas[i], "\".names2concepts\n",
